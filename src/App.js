...
function App() {
  ...
  */
  const handleSubmit = (e) => {
    ...
    if (!name) {
      ...
      setEditID(null);
      setIsEditing(false);
      showAlert(true, 'success', 'Todo güncellendi');
    } else {
      /*
      ...
      */
      showAlert(true, 'success', 'Listeye 1 todo eklendi');
  ...
  const clearList = () => {
    showAlert(true, 'danger', 'Todo temizlendi');
    setList([]);
  }
  ...
  */
  const removeItem = (id) => {
    showAlert(true, 'danger', 'Todo silindi');
      ...
      */}
      {list.length > 0 && (
        <div className='grocery-container'>
          ...
          <button className='clear-btn' onClick={clearList}>Temizle</button>
        </div>
      )}
